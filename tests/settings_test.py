...
class Logic(unittest.TestCase):
    ...
    def test_catalog(self):
        ...
        shipped_desktop_dir = ROOT / "applications"
        for entry in s.ENTRIES:
            self.assertIn(entry.category, s.CATEGORIES)
            self.assertTrue(entry.target)
            if not entry.desktop:
                # A desktop=False entry's script lives in HyDE's own repo, not
                # here, so its presence can't be verified from this repo alone
                # -- only that the command shape itself is right.
                self.assertEqual(s.hyde_command(entry)[:6], ["hyde-shell", "app", "-t", "scope", "--", "hyde-shell"])
            elif (shipped_desktop_dir / entry.target[0]).exists():
                # This repo ships this desktop file itself instead of relying
                # on a system package's -- keep it in sync with its Entry
                # target. Whether HyDE's own dots manifest actually deploys it
                # is HyDE's concern (checked over there), not this repo's.
    ...
    def test_default_apps_targets_a_standalone_tool(self):
        ...
        path = ROOT / "applications" / entry.target[0]
...
class GtkBehaviour(unittest.TestCase):
    ...
    def test_weather_location_refreshes_waybar_on_selection(self):
        # The bug this guards: selecting a location saved the new
        # coordinates but never told Waybar to refresh, so custom-weather.jsonc
        # (polls every 3600s, listens on signal 10) kept showing the previous
        # location for up to an hour.
        app = s.create_application()
        app.theme_path = self.path
        app.do_activate()
        ctx = {}
